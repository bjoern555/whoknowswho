...
app.post('/api/register', (req, res) => {
  const { name, color, emoji, drunkLevel, photo } = req.body
  if (!name) return res.json({ message: 'Name is required' })

  const db = readDB()
  const exists = db.people.find(p => p.name.toLowerCase() === name.toLowerCase())
  if (exists) {
    if (drunkLevel !== undefined && drunkLevel !== null) exists.drunkLevel = drunkLevel
    if (photo !== undefined) exists.photo = photo
    writeDB(db)
  ...
  const newPerson = { id: Date.now().toString(), name, color: color || '#4a90d9', emoji: emoji || '', drunkLevel: drunkLevel ?? null, photo: photo || '', knows: [] }
...
app.patch('/api/people/:id', (req, res) => {
  const { color, emoji, drunkLevel, photo } = req.body
  const db = readDB()
